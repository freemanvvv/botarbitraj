...
**Чистка модели (SuperSplat, локально).** Редактор Gaussian Splat SuperSplat
(MIT) можно поставить у себя — обрезать шум/floaters и экспортировать чистый
`.ply`:

```bash
bash scripts/setup_supersplat.sh   # клонирует+собирает в webapp/supersplat/dist (пару минут)
```

После этого перезапусти сервер: SuperSplat отдаётся на `/supersplat`, а в 3D-
вкладке у каждой модели появляется кнопка «✂️ Очистить в SuperSplat» (открывает
редактор с уже загруженной моделью — тот же origin, без CORS).
