...
def chat_endpoint(req: ChatRequest):
    ...
    try:
        ...
        broad = False           # запрос широкий → лучше уточнить, чем «выжимка»
        clarify_doc_list = ""   # список найденных нормативов для уточнения
        ...
        if req.use_rag:
            ...
            if relevant:
                ...
                # Широкий/неоднозначный запрос: релевантные фрагменты разбросаны
                # по МНОГИМ разным нормативам без явного лидера. Тогда «выжимка»
                # бесполезна — нужный свод правил утонет в объёме. Вместо ответа
                # просим сузить: показываем список документов и задаём
                # уточняющий вопрос (жалоба: «в 100 местах инфа, отдаёт целый
                # документ на 100 страниц — как найти один свод правил»).
                ctx_docs: dict = {}
                for r in relevant[:8]:
                    m = r.get("meta", {})
                    k = f"{m.get('doc_type','')} {m.get('number','')}".strip()
                    ctx_docs[k] = ctx_docs.get(k, 0) + 1
                ranks_desc = sorted((e["rank"] for e in by_doc.values()), reverse=True)
                clear_winner = len(ranks_desc) >= 2 and (ranks_desc[0] - ranks_desc[1]) >= 0.15
                if len(ctx_docs) >= 4 and not clear_winner:
                    broad = True
                    clarify_doc_list = "\n".join(
                        f"- {e['doc_type']} {e['number']} — {e['title']}".strip()
                        for e in sorted(by_doc.values(), key=lambda x: x["rank"], reverse=True)[:8]
                    )
        ...
        needs_clarify = False
        if broad:
            # Нашлось много разных нормативов без явного лидера → не «выжимка»,
            # а уточнение: показываем список документов и просим сузить.
            needs_clarify = True
            context = ("По запросу подходит сразу несколько разных нормативных "
                       "документов:\n\n" + clarify_doc_list + "\n")
            sys_prompt = (
                "Ты — Construction AI Copilot, ассистент по строительным нормам Узбекистана. "
                "Запрос пользователя ШИРОКИЙ: подходящая информация есть сразу в НЕСКОЛЬКИХ "
                "разных нормативах (они перечислены в контексте и показаны пользователю "
                "списком источников). НЕ пиши общую выжимку и НЕ пересказывай все документы "
                "подряд — из такого объёма пользователь не найдёт нужный свод правил. "
                "Вместо этого: (1) в 1–2 предложениях скажи, что по теме есть несколько "
                "нормативов; (2) задай 1–2 КОРОТКИХ уточняющих вопроса, чтобы сузить поиск — "
                "о каком объекте речь (жилое, общественное, промышленное здание или "
                "сооружение), какой именно аспект интересует (пожарная безопасность, "
                "эвакуация, инженерные сети, отделка, основания, теплозащита и т.п.), на "
                "каком этапе (проектирование, приёмка); либо предложи выбрать конкретный "
                "документ из списка источников. Будь кратким и дружелюбным, без выдумок."
            )
        elif context and not archive_only:
