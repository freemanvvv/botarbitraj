...
def _apply_polygon_template(tpl: dict, rooms_meta: list[tuple], fw: float, fd: float) -> tuple[dict, dict]:
    ...
    ЖЁСТКИЙ МАСШТАБ. Полигоны шаблона нормированы 0..1 по КАЖДОЙ оси
    независимо (конвертер: (px-x0)/span_x, (py-y0)/span_y) — истинные
    пропорции плана сохранены только в поле tpl["aspect"] = span_x/span_y.
    Если растянуть их прямо под запрошенные fw×fd (fw≠fd·aspect), форма
    искажается неравномерно: стена в «сжатой» оси визуально короче стены в
    «растянутой», хотя подпись честная — план выглядит непропорциональным.
    Поэтому реальный габарит комнат берём с СОХРАНЕНИЕМ aspect шаблона:
    площадь пятна оставляем как у запрошенного footprint'а (A=fw·fd, чтобы
    площадь этажа и объём 3D не изменились), но стороны пересчитываем под
    истинное соотношение — ew=√(A·a), ed=√(A/a), ew/ed=a. Тогда обе оси
    масштабируются ОДНИМ коэффициентом относительно нормировки, форма не
    искажается, и 1 м стены везде занимает одинаковую длину на чертеже."""
    area = max(fw * fd, 1e-6)
    aspect = tpl.get("aspect")
    if not aspect or aspect <= 0:
        aspect = (fw / fd) if fd else 1.0
    fw = math.sqrt(area * aspect)
    fd = math.sqrt(area / aspect)
