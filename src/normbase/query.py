"""
Поиск по нормативной базе с цитированием (NormBase, адаптирован).
"""
...
def build_where(status_mode: str):
    ...
    return {"status": {"$ne": nb_config.DEFAULT_EXCLUDE_STATUS}}


def retrieve(question: str, n: int, status_mode: str) -> List[Dict]:
    q_emb = nb_embed.embed_query(question)
    collection = nb_store.get_collection()
    return nb_store.query(collection, q_emb, n_results=n, where=build_where(status_mode))
...
def ask_llm(question: str, hits: List[Dict]) -> str:
    ...
    r = requests.post(f"{nb_config.LMSTUDIO_BASE_URL}/chat/completions",
                     json=payload, timeout=nb_config.REQUEST_TIMEOUT)
    r.raise_for_status()
