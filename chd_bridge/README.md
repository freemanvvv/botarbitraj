...
**Статус: подтверждено работающим на реальных весах** (Apple Silicon /
MPS, чекпоинт из Tsinghua Cloud, ~4с на 50 шагов диффузии) — см. раздел
про цветовые коллизии ниже, обе гипотезы из докстрингов подтверждены.
...
1. Склонируйте `chathousediffusion` и поставьте их зависимости в
   ...
   их авторами; на Apple Silicon (MPS) подтверждённо работает после
   4 точечных патчей (проверено на реальных весах, 50 шагов диффузии
   ~4с на MPS):
   - `trainer.py`: устройство `"cuda"` захардкожено — заменить на
     динамический выбор MPS/CPU/CUDA
   - `t5.py`: `.cuda()` → `.to(device)`
   - `denoising_diffusion_pytorch/image_process.py`: `float64` → `float32`
     (MPS не поддерживает double)
   - `predict.py`: `CUDA_VISIBLE_DEVICES=0` → `DEVICE = "mps"`
...
**Подтверждено на реальных весах (не только по чтению исходников):** их
`Trainer.predict()` действительно возвращает уже готовое RGB-изображение
(не индексы классов) — см. подробный докстринг в начале `predict_floorplan.py`.
Их собственная палитра не инъективна: несколько классов комнат окрашены
...
входа и двери (`FrontDoor`) — на реальном предсказании эвристика
корректно выделяет класс 10 (Entrance) из коллизии 0/10/12; без записи
EXTERNAL в палитре нearest-lookup весь фон/стены на реальном
предсказании тоже подтверждённо схлопывались бы в один ложный LivingRoom.
Остальные коллизии безобидны для нашей таксономии (см.
`src/floorplan/vectorize.py:CHATHOUSEDIFFUSION_CLASS_TO_TYPE` — несколько
их классов и так сводятся к одному нашему типу).
